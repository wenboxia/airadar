...
- **[2026-09-18]** 打分标准与收录标准对齐（D45）：triage 先判 kind、营销通稿按类型封顶价值分、权重经交叉验证重定（信源 0.55→0.4，长期价值 0.25→0.5）；
  ...
  论文入口改成"被别处提到才抓"（D36 落地），正文抽取保留外链，feed 官方标签入库
...
**当前指标**：20 天连续定时运行 20 次全部成功 · 122 个测试全绿 · 规则校验 0 违规 · 分类主类一致率 93%（V4.1 Flash；V4 Pro 时 94%）、官网标签命中 88%。
打分标准改版后（离线评测，V4 Pro）：自动发布认同 64% · 自动丢弃 98% · 送审里该收 47%；换成现主力 V4.1 Flash 后同一批：73% · 94% · 44%，漏杀 1→3（D48）。
黄金集 111 条：自动发布认同 62% · 自动丢弃认同 95%（漏 1 条）· 送审里该收的 28%（区间都很宽，只看方向）。

**下一步**：每周一处理三块审批单；抽查攒够样本后看 Flash 的自动发布撤下率（D48）；arXiv 按作者查询待论文入口跑两周后再评估（见 todo.md）。
