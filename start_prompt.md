...
**AIRadar：每日定时运行的 AI 行业情报工作流**（按 Anthropic 的分法是提示链 + 路由，不是 agent，见 D49）——从权威信源抓取 → 信源分层筛选 → LLM 摘要分类 → 置信度路由（自动发布 / 人工审核 / 丢弃）→ 沉淀进可检索的知识库 → 前端仪表盘展示。整条 pipeline 配有评测集、准确率报告和 HITL 反馈回流。
...
2. **面试可讲**：每个设计决策都要能转化为面试谈资，全部记录在 `docs/decisions.md`（已 50 条，开头有故事线）。堆名词、过度设计是天敌——"能清醒地说出为什么不用 LangGraph"比"用了 LangGraph"更有价值
...
每日 GitHub Actions cron（05:00 北京时间）
        │
        ▼
pipeline/main.py（手写编排：按序跑 stages，统一错误隔离与预算控制）
...
3. `todo.md` — 当前进度与主人待办
4. `docs/decisions.md` — D1–D50 全部设计决策（**改架构前必读，别推翻已有结论**）
...
- **[2026-09-18]** arXiv 按作者查询量过之后决定不做（4 人近 90 天 18 篇，没有一篇有人讨论）；量的过程中修了论文闸门三处：
  运行统计被冲掉、25 人署名规则因截断失效、技术报告不看时间（2025 年的 R1 被当新闻发了，现在只放 14 天内的）；复核时又修了接口 30 个名额先给老论文、人名被当成机构名两处（D36 追加）
- **[2026-09-26]** 对外定位从「情报 Agent」改成「情报工作流」，和简历一致（D49）；评估过要不要做成多智能体编辑部，结论是暂不做（D50）。
  README 重做（中英两版、横幅、截图、运行数据图），运行数据改由 `tools/run_stats.py` 生成、标截止日；网站右上角加回 GitHub 入口；加 MIT 许可证

**当前指标**：定时运行 29 次全部成功（截至 09-25 UTC，`tools/run_stats.py --check-gh`）· 128 个测试全绿 · 规则校验 0 违规（824 条） · 分类主类一致率 93%（V4.1 Flash；V4 Pro 时 94%）、官网标签命中 88%。
...
**下一步**：每周一处理三块审批单；抽查会持续给出 V4.1 Flash 自动发布的撤下率，攒到 30 条以上再看要不要动门槛（D48）；论文闸门修好后开始积累"每天放进几篇"的运行记录（D36 追加）。
