...
#
# 0.3.1（同日）：客户案例被「Agent 与开发」吃掉，改了两段的入选句和一句判定句。
# 根因是两个类的入选句不同构——「Agent 与开发」写的是正文必然出现的技术名词（上下文、多智能体、API），
# 「产品与应用」写的是要先推断主角的角色短语，于是案例稿在前者逐词命中、在后者 0 命中。
# 原先「给开发者用的工具归 Agent 与开发」还选错了轴：Cursor / Warp 是开发者工具但该算案例，
# 而"某公司用 Codex 发了个消费级 App"该算 Agent 与开发。判据换成"读者拿走的是做法还是谁做成了什么"。
...
- Agent 与开发：Agent 架构、工具调用、MCP、多智能体、上下文与记忆、RAG；
  编程 Agent 的搭法与工作流（教程、作者讲自家怎么做的）；开发者 SDK / API 的发布与变更；
  AI 系统的部署运维
  不放：讲另一家公司用模型做成了什么、效果如何，归产品与应用；面向用户的产品功能、AI 用在某个行业里，
  也归产品与应用；Agent 被攻击或作恶归安全；Agent 基准归评测
...
- 产品与应用：面向用户的产品和功能上线；客户案例——某家公司 / 团队用某个模型做出了什么、用得怎么样
  （哪怕做出来的是给开发者用的工具）；AI 用于科研 / 医疗 / 金融
  不放：开发者工具自己发新版、开源新库，归 Agent 与开发；模型本身的发布归模型；公司融资和竞争归行业动态
- 行业动态：融资、收购、人事、算力和芯片、调价、市场竞争、政策监管、诉讼、社会影响
  只有其他 5 类都不是主角时才选它；"这是一条新闻"不是选它的理由

三个容易混的地方：
- 测模型做安全任务的能力算评测；测模型或系统会不会出事算安全
- 讲"怎么搭、怎么用"算 Agent 与开发；讲"某家公司用它做成了什么"算产品与应用。
  两边都像时看读者拿走什么：拿走可照搬的做法（架构、流程、工具链、踩坑）算 Agent 与开发；
  只拿走"谁做成了什么、效果多少"算产品与应用，正文里的技术细节只是佐证。
  《我们怎么用 Codex 28 天发了安卓版》算 Agent 与开发，《Cursor 怎么用 GPT-5》算产品与应用
