...
# 三条铁律都是被真实幻觉打出来的（decisions.md D18、D34）：
# ① 元数据不是内容——模型曾把信源名写成文章作者（"Simon Willison 引用了…"，原文根本没提他）
#    D34 补充：光靠指令挡不住。**现在干脆不把信源名传给模型**——
#    写摘要用不到它（标题+正文足够），不给它这个信息，泄漏就不可能发生
# ② 不许外推——"可能改变推理硬件格局"这类结论是模型自己加的
# ③ 不许补背景——模型曾从 HN 标题编出整段"CEO 解雇开发者"的故事
_RULES = """铁律（违反任意一条都算失败）：
1. 只依据【原文】写作。标题是元数据、**不是原文内容**。
   **绝不许自行补充发布方或作者**——原文没署名就不写，不许根据你的猜测填上机构名。
...
def _summarize_one(it, ctx: Context) -> bool:
    ...
    # **不传信源名**：模型拿不到就泄漏不了（D34）。
    # 前一版把它标为"元数据（仅供参考）"仍然被写进摘要——
    # 结构上不给，比指令上禁止可靠得多
    out = ctx.llm.json_chat(
        _SYS_SUM,
        f"【标题（元数据，不是文章内容）】{it.title}\n"
