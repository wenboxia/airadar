...
**线上**：https://wenboxia.github.io/airadar/ ｜ 已连续定时运行 **19 天，19 次全部成功**（2026-08-29 → 09-16，GitHub Actions 运行记录）
...
口径：`data/runs/` 里 2026-08-29 起的 19 次定时运行逐次累加，截至 09-16。

| 指标 | 值 |
|---|---|
| 累计抓取 → 去重后进入打分 | 927 → 472 条（筛掉 49%） |
| 三路去向 | 自动发布 216 · 送人工 220 · 自动丢弃 36 |
| LLM 调用 | 1,482 次，291 万 token |
| 切到备用模型 | 58 次（3.9%），19 天里有 18 天发生过 |
| 非致命错误 | 30 个，全部是单个信源抓取失败，没有一次阻塞整条流水线 |
| 单次运行耗时 | 中位 12.4 分钟 |
| 知识库 | 已发布 262 条，其中 28 条时效内容已按规则退出 |
| 回归测试 | 62 个 |

## 评测：方法和真实结果

- **规则校验**：字段完整性、状态合法性，每次运行必跑。当前 550 条 0 违规。
- **黄金集**：人工标注的标准答案，按"自动发布 / 自动丢弃 / 送人工"三条路径分别评（D29）。
  v1 共 26 条，测出**自动发布精确率 67%（15 条）、送人工命中率 20%（10 条）**。
  这两个数字推动了后来的改动，但 v1 没有成文标准、收录了太多普通论文，已整体作废（D35）。
  **v2 共 100 条，已随机分层采样、资料已准备好，正在人工标注——所以这里暂不报准确率。**
...
AI 只做资料准备：读原文、写"讲了什么"、标信息类型。**是否收录由人判断，模型不参与**——被评测的系统本身就是模型在打分，标准答案再让模型来给，就成了模型评模型。
写资料的模型（Kimi / GLM）也刻意和负责打分的 DeepSeek 不是同一家。标准和版本记录见 [evals/golden_set/README.md](evals/golden_set/README.md)。

## 设计决策

[docs/decisions.md](docs/decisions.md) 记录了 41 条决策，开头有按五段组织的故事线。最值得先看的三条：
...
python3 -m unittest discover evals            # 62 个回归测试
```
