...
- **黄金集 v2**：111 条人工判断（收录 39 条），按"自动发布 / 自动丢弃 / 送人工"三条路径分别评（D29）：
  ...
  | 系统的判断 | 09-16 的打分标准 | 改完之后（离线重打分） |
  |---|---|---|
  | 自动发布里我认同的 | 62%（37 条） | **64%**（33 条） |
  | 自动丢弃里我也认为该丢的 | 95%（20 条） | **98%**（42 条） |
  | 送审里我最终会收的 | 28%（54 条） | **47%**（36 条） |

  送审命中率从 28% 到 47%，靠的不是调阈值，是**让打分标准和收录标准变成同一套**（D45）：
  以前 triage 只看"跟四个领域贴不贴"，而我标注时用的是"一手还是转述、是不是营销通稿、有没有可复用的方法"。
  按内容类型看最直观——**19 条营销通稿里 17 条被直接丢弃、0 条自动发布**，而我一条都没收。
...
- **分类**：8 个平铺的类（模型 / Agent 与开发 / 评测 / 安全 / 产品与应用 / 行业动态 / 开源项目 / 研究论文），后两个按链接判定（D43）。两条线都是跑之前定的：
  ...
  | 测什么 | 怎么测 | 结果 |
  |---|---|---|
  | 噪声底 | 同一个 prompt 对同样 100 条连分两次，主类一致的比例 | 旧 13 类 73% → 新 8 类 **94%**（验收线 85%） |
  | 准确率 | OpenAI / Anthropic 官网 feed 自带的标签当参照，18 个标签共 195 条 | 主类命中 **88%**（95% 区间 82%–92%） |
...
[docs/decisions.md](docs/decisions.md) 记录了 47 条决策，开头有按五段组织的故事线。最值得先看的三条：
...
- **D47** 三家模型选型的决定性差异不是准确率，是完成率：Kimi 111 条里 73 条被限流打回（[eval_report.md](docs/eval_report.md)）
...
python3 -m unittest discover evals            # 116 个回归测试
```
