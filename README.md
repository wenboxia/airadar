...
```mermaid
flowchart LR
    SRC["20 个分层信源 + 论文闸门<br/>S 官方 · A 专家 · B 媒体 · C 跨界"] --> F["fetch 抓取"]
    F --> D["dedupe 去重"] --> T{"triage 打分<br/>等级 40% + 模型 60%"}
...
- **规则校验**：字段完整性、状态合法性，每次运行必跑。截至 09-18 共 608 条，0 违规。
...
[docs/decisions.md](docs/decisions.md) 记录了 48 条决策，开头有按五段组织的故事线。最值得先看的六条：
