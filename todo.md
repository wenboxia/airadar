...
- [ ] **和 Claude 定分类体系**：已定模型合并、安全合并、「开源项目」「研究论文」保留（按链接判定）；
      草案 8 类：模型 / Agent 与开发 / 评测 / 安全 / 产品与应用 / 行业动态 / 开源项目 / 研究论文。
      下一步 Claude 给出每类「放什么 / 不放什么」，确认后再动手改
- [ ] 每周一处理审批 issue（12 条，勾完点 Close）。Issue #3 已处理（09-17），09-18 的运行会回收

已完成：GLM、DeepSeek 充值（09-16）· 黄金集 v2 标注（09-17，共 111 条，见 D42）
...
- [ ] 8 类边界句给主人确认
- [ ] classify.py 类目表与 prompt → 同一 prompt 重跑一致率基线（验收 85%）→ 回填（`categories_v2` 快照）→ 前端筛选 → 测试 → D43
- [ ] 分类准确率：用 OpenAI / Anthropic feed 自带标签映射成评测集
