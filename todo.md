...
- [x] ~~给 GLM、DeepSeek 充值~~（09-16 已充，GLM 测试调用正常，DeepSeek 余额 ¥61.90）
- [ ] **标注黄金集 v2**：打开 `evals/golden_set/黄金集v2_标注表.docx`，只填最右边三列（收录 y/n、分类、一句理由）。
      100 条全部填完后跑一次 `python3 evals/import_golden_docx.py`（把 Word 里的结果写进评测用的标准答案文件），
      再跑 `python3 evals/run_eval.py`（算出系统判断的准确率），然后告诉 Claude 开始第二批。
      60 条只是三家模型对比的最低样本量，不需要标到 60 就停下来
- [ ] **处理 [Issue #3](https://github.com/wenboxia/airadar/issues/3)**（12 条）—— 勾选后**必须点 Close issue** 才算提交。
      **最好在 9/21（周一）早 7 点前关掉**，那天才会开出下一张；最晚 9/28 早 7 点，过了会被自动按过期关闭
      （已勾的照样算通过，没勾的留在待审）。之后每周一一张，节奏相同
...
### Phase C：三家模型对比（黄金集 v2 标完后；60 条是最低样本量）
